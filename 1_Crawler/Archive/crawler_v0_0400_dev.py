...
def crawl_search_result(driver, term, page_limit=20):
...
def master_affiliate():
    ...
    aff_link = ar.gsheet_get_data()
    aff_link = aff_link[['link', 'aff_link']]
    
    item_link = ar.gsheet_get_data()
    
    
    loc_df = cbyz.df_anti_merge(item_link, aff_link, on='link')
    loc_df = loc_df.reset_index(drop=True)
    
    for i in range(len(loc_df)):
    	driver.get(loc_df.loc[i, 'link']) 
