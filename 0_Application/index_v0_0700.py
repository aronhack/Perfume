...
def dashboard(brand=[], name=[], top_note=[], heart_note=[], base_note=[], 
              status=0):
    ...
    if len(brand) > 0:
        brand_items = ms.perfume[ms.perfume['brand'].isin(brand)]
        brand_items = brand_items[['id']].drop_duplicates()
        target_id_df = target_id_df.merge(brand_items, on='id')
    ...
    perfume_list = perfume_list[['brand', 'name',
                                 'top_note', 'heart_note', 'base_note']]
    
    perfume_list.columns = ['品牌', '名稱', '前調', '中調', '後調']
    
    
    cols = [{"name": i, "id": i} for i in perfume_list.columns]
    cols[1]['presentation'] = 'markdown'
...
def update_note(brand, top_note, heart_note, base_note, 
                clicks, status):
    ...
    tb_data, tb_cols, heatmap_data, heatmap = \
        dashboard(brand, [], top_note, heart_note, base_note, status)
